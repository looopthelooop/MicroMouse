...
void rt_OneStep(void)
{
  ...
  __enable_irq();
  currentTime = (extmodeSimulationTime_T) testing_M->Timing.taskTime0;
  testing_step();
...
int main(int argc, char **argv)
{
  ...
  testing_configure_interrupts();
  testing_initialize();
  __disable_irq();
  __enable_irq();

  /* External Mode initialization */
  errorCode = extmodeInit(testing_M->extModeInfo, &rtmGetTFinal(testing_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  ;
  testing_unconfigure_interrupts();
