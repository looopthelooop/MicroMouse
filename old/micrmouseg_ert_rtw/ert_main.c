...
int main(int argc, char **argv)
{
  ...
  MX_TIM1_Init();
  MX_TIM16_Init();
  rtmSetErrorStatus(micrmouseg_M, 0);
  ...
  micrmouseg_configure_interrupts();
  micrmouseg_initialize();
  __disable_irq();
  __enable_irq();

  /* External Mode initialization */
  errorCode = extmodeInit(micrmouseg_M->extModeInfo, &rtmGetTFinal(micrmouseg_M));
  if (errorCode != EXTMODE_SUCCESS) {
  ...
  if (errorCode == EXTMODE_SUCCESS) {
    ...
    if (extmodeStopRequested()) {
      rtmSetStopRequested(micrmouseg_M, true);
    }
  }
  ...
  runModel =
    ...
    !rtmGetStopRequested(micrmouseg_M);
  __enable_irq();
  __enable_irq();
  while (runModel) {
  ...
  ;
  micrmouseg_unconfigure_interrupts();
